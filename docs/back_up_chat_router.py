...
async def _save_tool_message(conv_mgr, msg_dict):
    ...
    if updated_tc:
        logger.info(f"Updated tool_call {tool_call_id} ({name}) with output")
...
async def check_and_handle_interrupts(agent, langgraph_config, make_chunk, meta, thread_id):
    ...
    try:
        ...
        if interrupt_info:
            ...
            # logger.info(f"----->question : {interrupt_info.get("question")}")
            # logger.info(f"----->operation: {interrupt_info.get("operation")}")
            ...
            if isinstance(interrupt_info, dict):
                ...
                logger.info(f"1----->operation: {operation}")
                ...
                if isinstance(first_interrupt, dict):
                    ...
                    logger.info(f"2----->operation: {operation}")
                else:
                    operation = str(first_interrupt)
                    logger.info(f"3----->operation: {operation}")
            else:
                operation = str(interrupt_info)
                logger.info(f"4----->operation: {operation}")

            #从中提取出data数据
            data_value=extract_data_try(operation)
            logger.info(f"----->data_value: {data_value}")
            # 发送人工审批请求到前端
            logger.info(f"Sending human approval request - question: {question}, operation.value: {operation}")

            yield make_chunk(
                status="human_approval_required",
                thread_id=thread_id,
                interrupt_info={"question": question, "data": data_value ,"operation": "interrupt"},
...
async def resume_agent_chat(
    ...
    async def stream_resume():
        ...
        logger.info(f"----->准备恢复中断  Resuming agent_id: ")
        # 使用 Command(resume=approved) 恢复执行 action="proceed" if decision else "cancel"

        # resume_command = Command(resume={"action": "approve" if approved else "cancel", "subject": "Updated subject"})

        resume_command = Command(resume={"decisions": [{"type":"approve"}]  if approved else [{"type":"reject"}], "subject": "Updated subject"})
        ...
        user_id=100022
        input_context = {"user_id": str(user_id), "thread_id": thread_id}
        ...
        try:
            async for msg, metadata in stream_source:
                # 确保msg有正确的ID结构
                msg_dict = msg.model_dump()
                # logger.info(f"----->stream msg  {msg_dict} ")
...
def extract_data_try(operation_str:str):
    # text = """Interrupt(value={'action': 'send_email', 'data': [{'name': '发送给(to)', 'key': 'to', 'value': 'detaff_14@dev_depart.com', 'type': 'string', 'required': True}, {'name': '主题(subject)', 'key': 'subject', 'value': '关于明天全员会议的通知', 'type': 'string: True}, {'name': '内容', 'key': 'body', 'value': '14号全员会议,公司开发部二楼会议室，讨论新项目设计方案', 'type': 'string', 'required': True}], 'message': 'hello'}, id='1be7705be3f1045fa6c5e2eb956087cc')"""

    # 1. 提取 data 数组
    pattern = r"'data'\s*:\s*(\[[\s\S]*?\])"
    match = re.search(pattern, operation_str)
    data_str = match.group(1)

    # 2. 修复常见语法错误
    # 修复错误的 type 字段
    data_str = data_str.replace("'type': 'string: True", "'type': 'string', 'required': True")

    # 3. 将 Python 风格的 dict 字符串转成 Python 对象
